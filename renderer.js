// ==========================================
// 🎵 Playly - Renderer Process Logic v2
// ==========================================
...
/**
 * Конвертирует base64 обложку в Blob URL для быстрой отрисовки
 */
function base64ToBlobUrl(base64, mimeType) {
    ...
    try {
        ...
        console.warn('Failed to convert cover to blob:', e);
        return null;
    }
}

/**
 * Освобождает память от Blob URL
 */
...
let _loadedCovers = new Set(); // Кэш загруженных обложек

function initCoverObserver() {
  if (_coverObserver) return;
  _coverObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (!entry.isIntersecting) return;
      const img = entry.target;
      const coverData = img.dataset.coverData;
      if (coverData && !_loadedCovers.has(coverData)) {
        img.src = coverData;
        _loadedCovers.add(coverData);
        img.onload = () => {
          delete img.dataset.coverData;
        };
        img.onerror = () => {
          delete img.dataset.coverData;
        };
      } else if (_loadedCovers.has(coverData)) {
        // Уже загружена - ставим сразу
        img.src = coverData;
        delete img.dataset.coverData;
      }
...
async function init() {
  ...
  // Откладываем тяжёлые фоновые задачи
...
function setupEventListeners() {
  ...
  // Кнопки на карточке "Сейчас играет"
  document.getElementById('npPlayBtn')?.addEventListener('click', togglePlay);
  document.getElementById('npPrevBtn')?.addEventListener('click', handlePrev);
  document.getElementById('npNextBtn')?.addEventListener('click', handleNext);
  document.getElementById('npShuffleBtn')?.addEventListener('click', () => { toggleShuffle(); updateNowPlayingCard(); });
  document.getElementById('npRepeatBtn')?.addEventListener('click', () => { toggleRepeat(); updateNowPlayingCard(); });
  // Кликабельный прогресс-бар на карточке
  document.getElementById('nowPlayingProgressBg')?.addEventListener('click', (e) => {
  ...
  // Оптимизация: дебаунс для resize чтобы не лагало при fullscreen
  let resizeTimer = null;
  window.addEventListener('resize', () => {
    clearTimeout(resizeTimer);
    resizeTimer = setTimeout(() => {
      // Ничего не делаем - карточки адаптируются через CSS grid
    }, 250);
  }, { passive: true });
  ...
  // Оптимизация: отключаем transitions во время прокрутки для плавности
  let scrollTimer = null;
  const playlistView = document.getElementById('playlistView');
  if (playlistView) {
    playlistView.addEventListener('scroll', () => {
      playlistView.classList.add('is-scrolling');
      clearTimeout(scrollTimer);
      scrollTimer = setTimeout(() => {
        playlistView.classList.remove('is-scrolling');
      }, 150);
    }, { passive: true });
  }
  ...
  // === ДЕЛЕГИРОВАНИЕ СОБЫТИЙ ДЛЯ ПЛЕЙЛИСТА ===
  if (playlistGrid) {
    playlistGrid.addEventListener('click', (e) => {
      ...
      if (menuBtn) {
        e.stopPropagation();
        contextTrackIndex = parseInt(menuBtn.dataset.index);
    ...
    playlistGrid.addEventListener('contextmenu', (e) => {
      const card = e.target.closest('.track-card');
      if (card) {
        e.preventDefault();
        contextTrackIndex = parseInt(card.dataset.index);
...
async function playTrack(index) {
    ...
    // Если трек уже загружен — просто включаем
    if (index === currentIndex) {
        togglePlay();
        return;
    }
    
    // Иначе загружаем новый (async)
    await loadTrack(index);
    play();
}
// ===== РЕНДЕРИНГ ПЛЕЙЛИСТА В ФОНЕ (для модалки загрузки) =====

async function renderPlaylistInBackground() {
    if (!playlistGrid) return;
    if (!window._cardPool) await initCardPool();
    ...
    for (let start = 0; start < pageTracks.length; start += BATCH) {
        ...
        for (let i = start; i < end; i++) {
            ...
            if (realIndex !== undefined && window._cardPool[i]) {
                updateTrackCard(window._cardPool[i], track, realIndex);
                window._cardPool[i].style.display = 'flex';
            }
        }
        rendered = end;
...
// ===== ЗАГРУЗКА МЕТАДАННЫХ В МОДАЛКЕ (с Blob конвертацией) =====
...
async function loadMetadataInModal(newTracks, totalFiles) {
    ...
    async function metaWorker() {
        while (idx < newTracks.length) {
            ...
            if (!track || track._metaLoaded || track.path.startsWith('demo-')) { 
                done++; 
                continue; 
            }
...
// Пул DOM-элементов для переиспользования (Object Pooling)
window._cardPool = null;

// Вспомогательная функция для плавного батчинга
const scheduleBatch = window.requestIdleCallback 
  ? (fn) => requestIdleCallback(fn, { timeout: 100 })
  : (fn) => setTimeout(fn, 32); // ~2 кадра при 60fps

// Инициализация пула карточек асинхронно (вызывается один раз)
// ===== ИНИЦИАЛИЗАЦИЯ ПУЛА КАРТОЧЕК =====

async function initCardPool() {
    if (window._cardPool) return;
    
    window._cardPool = [];
    const CHUNK_SIZE = 15;
    
    for (let start = 0; start < PAGE_SIZE; start += CHUNK_SIZE) {
        const end = Math.min(start + CHUNK_SIZE, PAGE_SIZE);
        ...
        for (let i = start; i < end; i++) {
            ...
            card.innerHTML = `
                ...
                <div class="track-card-info">
                    <div class="track-card-title"></div>
                    <div class="track-card-meta">
                        <span class="track-card-type"><i class="fa-regular fa-file-audio"></i> <span></span></span>
                        <span class="track-card-duration"><i class="fa-regular fa-clock"></i> <span></span></span>
                        <span class="track-liked-badge" style="display:none;"><i class="fa-solid fa-heart"></i></span>
                    </div>
                </div>
                <div class="track-card-menu">
                    <button class="track-menu-btn"><i class="fa-solid fa-ellipsis"></i></button>
                </div>
            `;
            
            // === КЭШИРУЕМ ВСЕ УЗЛЫ для мгновенного доступа ===
            const art = card.querySelector('.track-card-art');
            const img = card.querySelector('.track-card-art img');
            const musicIcon = card.querySelector('.track-card-art .fa-music');
            const overlay = card.querySelector('.play-overlay');
            const overlayIcon = overlay.querySelector('i');
            const check = card.querySelector('.select-check');
            
            card._nodes = { art, img, musicIcon, overlay, overlayIcon, check };
            card._title = card.querySelector('.track-card-title');
            card._typeSpan = card.querySelector('.track-card-type span');
            card._durationSpan = card.querySelector('.track-card-duration span');
            card._likedBadge = card.querySelector('.track-liked-badge');
            card._menuBtn = card.querySelector('.track-menu-btn');
            
            // Сохраняем текущее состояние для оптимизации
            card._currentCoverUrl = null;
            card._isShowingCover = false;
            card._isShowingCheck = false;
            
            window._cardPool.push(card);
...
function updateTrackCard(cardElement, track, realIndex) {
    ...
    const nodes = cardElement._nodes;
    
    // === 1. Классы и data-атрибуты ===
    cardElement.className = 'track-card' + (isActive ? ' active-track' : '') + (isSelected ? ' selected' : '');
    ...
    if (track.cardColor && !isSelected) {
        ...
        cardElement.style.background = `rgba(${r},${g},${b},0.12)`;
        cardElement.style.borderColor = `rgba(${r},${g},${b},0.35)`;
    } else {
...
async function renderPlaylist(page) {
  ...
  if (!displayTracks.length) {
    // Скрываем только карточки пула, показываем заглушку
    window._cardPool.forEach(card => card.style.display = 'none');
  ...
  // ГЛАВНЫЙ ФИКС: Обновляем карточки СРАЗУ, без скрытия всего списка
  for (let i = 0; i < PAGE_SIZE; i++) {
    const card = window._cardPool[i];
    ...
    if (i < pageTracks.length) {
      ...
      // Скрываем только лишние карточки (те, что за пределами страницы)
      if (card.style.display !== 'none') {
        card.style.display = 'none';
      }
    }
...
window.goToPage = function(page) {
  ...
  // Плавный скролл наверх перед рендерингом
  const pv = document.getElementById('playlistView');
  if (pv) {
    pv.style.scrollBehavior = 'auto';
    pv.scrollTop = 0;
    pv.style.scrollBehavior = '';
  }
...
// УСТАРЕВШАЯ ФУНКЦИЯ - заменена на updateTrackCard для DOM Recycling
// Оставлена для совместимости, но больше не используется
function buildTrackCard(track, realIndex, isSelecting) {
  console.warn('buildTrackCard is deprecated - use updateTrackCard with card pool instead');
  const isActive = realIndex === currentIndex;
  const isSelected = selectedTracks.has(track.path);
  const card = document.createElement('div');
  ...
  // Кэш больше не используется - работаем с пулом
  ...
  trackDetailsContent.innerHTML = `
    <div class="track-details-block">
      <div class="track-details-row name-row">
        ...
        <span class="track-details-value" title="${escapeAttr(track.name)}" style="max-width:none;text-align:right;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(track.name)}</span>
        <button class="copy-name-btn" data-copy="${escapeAttr(track.name)}" title="Копировать название">
    ...
    <div class="track-details-block">
      <div class="track-details-row">
        ...
        <div style="display:flex;align-items:center;gap:8px;flex:1;justify-content:flex-end;">
          ...
          <button class="copy-name-btn" data-copy="${escapeAttr(track.path)}" title="Копировать путь">
            <svg viewBox="0 0 24 24"><path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"/></svg>
          </button>
        </div>
      </div>
      <div class="track-details-row">
        ...
        <div style="display:flex;align-items:center;gap:8px;flex:1;justify-content:flex-end;">
          ...
          <button class="copy-name-btn" data-copy="${escapeAttr(folderPath)}" title="Копировать путь к папке">
            <svg viewBox="0 0 24 24"><path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"/></svg>
          </button>
        </div>
      </div>
...
function saveDataDebounced() {
  ...
  _saveDebounceTimer = setTimeout(() => { saveData(); }, 2000);
}

// ===== ЗАГРУЗКА МЕТАДАННЫХ (очередь, не зависает) =====
const META_CONCURRENCY = 1; // Уменьшил до 1 для плавности UI
let _metaQueue = [];
let _metaActive = 0;
...
function _metaWorker() {
  ...
  if (!track || track._metaLoaded) { 
    // Увеличил паузу между треками для плавности UI
    setTimeout(_metaWorker, 50);
    return; 
  }
  track._metaLoaded = true;
  _metaActive++;
  loadMetadataForTrack(track).then(() => {
    _metaActive--;
    // Увеличил паузу между загрузками для плавности UI
    setTimeout(_metaWorker, 100);
  });
}

// ===== ЗАГРУЗКА МЕТАДАННЫХ (с конвертацией обложек) =====
    ...
    try {
        ...
        if (meta) {
            ...
            if (meta.title && meta.title.trim()) { 
                track.name = meta.title.trim(); 
                changed = true; 
            }
            if (meta.artist && meta.artist.trim()) { 
                track.artist = meta.artist.trim(); 
                changed = true; 
            }
            if (meta.album) track.album = meta.album;
            ...
            if (changed) {
                // Отложенное обновление UI чтобы не блокировать рендер
                const updateUI = () => {
                    ...
                    if (tracks[currentIndex]?.path === track.path) {
                        ...
                        if (playerArt && track.coverBlobUrl) {
                    ...
                    // Обновляем карточку в плейлисте БЕЗ полного ре-рендера
                    const card = playlistGrid?.querySelector(`[data-path="${CSS.escape(track.path)}"]`);
                    if (card && typeof updateTrackCard === 'function') {
...
// ===== ЗАГРУЗКА ТРЕКА =====
// ===== ПОЛУЧЕНИЕ URL ДЛЯ ЛОКАЛЬНОГО ФАЙЛА (через IPC) =====

// Кэш URL чтобы не запрашивать каждый раз
const _fileUrlCache = new Map();

async function getFileUrl(filePath) {
    if (!filePath) return '';
    if (filePath.startsWith('demo-')) return filePath;
    if (filePath.startsWith('localfile://')) return filePath;
    
    // Проверяем кэш
    if (_fileUrlCache.has(filePath)) {
        return _fileUrlCache.get(filePath);
    }
    
    try {
        // Запрашиваем URL через IPC (main.js уже всё правильно кодирует)
        const url = await window.electronAPI?.getFileUrl?.(filePath);
        if (url) {
            _fileUrlCache.set(filePath, url);
            return url;
        }
    } catch (e) {
    ...
    // Fallback: старый метод (на случай если IPC не доступен)
    return buildFileUrlFallback(filePath);
}

// Резервный метод (если IPC не работает)
function buildFileUrlFallback(filePath) {
    if (!filePath || filePath.startsWith('demo-')) return filePath;
...
// Оптимизировано для работы с пулом карточек
function updateActiveCardIcon() {
  if (!playlistGrid || !window._cardPool) return;
  
  // Убираем выделение с предыдущей активной карточки
  if (_lastActiveCard) {
    _lastActiveCard.classList.remove('active-track');
    const overlay = _lastActiveCard.querySelector('.play-overlay i');
    if (overlay) overlay.className = 'fa-solid fa-play';
    _lastActiveCard = null;
  }
  
  // Находим новую активную карточку по индексу в пуле
  if (currentIndex !== -1 && tracks[currentIndex]) {
    ...
    for (let i = 0; i < window._cardPool.length; i++) {
      const card = window._cardPool[i];
      if (card.dataset.path === currentPath && card.style.display !== 'none') {
...
function startProgress() {
  ...
  progressInterval = setInterval(() => {
    if (audio.duration) {
      ...
      const npTot = document.getElementById('nowPlayingTimeTotal');
      if (npFill) npFill.style.width = pct + '%';
      if (npCur) npCur.textContent = formatTime(audio.currentTime);
      if (npTot) npTot.textContent = formatTime(audio.duration);
    }
  }, 100);
}
...
function getVolumePct(clientX) {
  const rect = volumeContainer.getBoundingClientRect();
  return Math.max(0, Math.min(1, (clientX - rect.left) / rect.width));
}

function applyVolumePct(pct) {
  // Snap to 0 when very close to avoid floating-point never reaching 0
  const vol = pct < 0.01 ? 0 : pct;
  audio.volume = vol;
...
function setupDraggableVolume() {
  volumeContainer.addEventListener('mousedown', (e) => {
    ...
    applyVolumePct(getVolumePct(e.clientX));
  });
  ...
  volumeContainer.addEventListener('touchstart', (e) => {
    isDraggingVolume = true;
    volumeContainer.classList.add('dragging');
    applyVolumePct(getVolumePct(e.touches[0].clientX));
  }, { passive: true });
...
window.switchPage = function(el) {
  ...
  [homeWrapper, playlist, settings].forEach(s => s?.classList.remove('active'));
...
function updateNowPlayingCard() {
  ...
  const timeCur = document.getElementById('nowPlayingTimeCurrent');
  const timeTot = document.getElementById('nowPlayingTimeTotal');
  ...
  if (currentIndex === -1 || !tracks[currentIndex]) {
    card.classList.remove('has-track', 'has-accent');
    card.style.removeProperty('--np-accent');
    card.style.removeProperty('--np-accent-bg');
    if (artEl) {
      ...
      if (!musicIcon) {
        ...
        artEl.insertBefore(musicIcon, artEl.firstChild);
      }
  ...
  // Цветной акцент из cardColor трека
  if (track.cardColor) {
    const hex = track.cardColor;
    const r = parseInt(hex.slice(1,3),16), g = parseInt(hex.slice(3,5),16), b = parseInt(hex.slice(5,7),16);
    card.classList.add('has-accent');
    card.style.setProperty('--np-accent', `rgba(${r},${g},${b},0.4)`);
    card.style.setProperty('--np-accent-bg', `rgba(${r},${g},${b},0.07)`);
  } else {
  ...
  if (npPlayIcon) npPlayIcon.className = isPlaying ? 'fa-solid fa-pause' : 'fa-solid fa-play';
  // Синхронизируем состояние shuffle/repeat
  const npShuffle = document.getElementById('npShuffleBtn');
  const npRepeat = document.getElementById('npRepeatBtn');
  if (npShuffle) npShuffle.classList.toggle('active', isShuffle);
  if (npRepeat) npRepeat.classList.toggle('active', repeatMode > 0);

  // Прогресс
  if (progressWrap) progressWrap.style.display = 'block';
  const dur = audio.duration || track.duration || 0;
  const cur = audio.currentTime || 0;
  if (progressFillEl) progressFillEl.style.width = dur ? (cur / dur * 100) + '%' : '0%';
